...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for (int i=0; i < userCount; i++)
        {
            User othUser = users[i];
            if(!othUser.getName().equals(name))
            {
            int mutual= othUser.countMutual(getUser(name));
                if (mutual> maxMutual)
            {
                maxMutual= mutual;
                maxUser= othUser;
    ...
    public String mostPopularUser() {
        ...
        for (int i=0; i<userCount; i++)
        {
           int tempCount = followeeCount(users[i].getName());
           if(tempCount>maxfCount)
           {
              maxfCount=tempCount;
              mostPopUser= users[i];
           }
